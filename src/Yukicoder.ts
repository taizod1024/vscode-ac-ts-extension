...
class Yukicoder implements Coder {
  ...
  async loginSiteAsync() {
    ...
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] yukicoder.apikey: ********`
    );
  }
  ...
  async getTestAsync() {
    ...
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] yukicoder.fileinurl: ${fileiurl}`
    );
    ...
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] -> ${resfilei.status}`
    );
    ...
    for (let nx = 0; nx < filei.length; nx++) {
      ...
      actsextension.channel.appendLine(
        `[${actsextension.timestamp()}] yukicoder.fileiurl-${nx}: ${fileixurl}`
      );
      ...
      actsextension.channel.appendLine(
        `[${actsextension.timestamp()}] yukicoder.fileourl-${nx}: ${fileoxurl}`
      );
  ...
  async submitTaskAsync() {
    ...
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] -> ${res3.status}`
    );
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] submissionsurl: ${this.submissionsurl}`
    );
  }

  browseTask() {
    actsextension.channel.appendLine(
      `[${actsextension.timestamp()}] taskurl: ${this.api_problemidurl}`
    );
  ...
  loadConfig(json: any) {
    yukicoder.apikey = json.yukicoder?.encapikey
      ? Buffer.from(json.yukicoder?.encapikey, "base64").toString()
      : "";
  }
