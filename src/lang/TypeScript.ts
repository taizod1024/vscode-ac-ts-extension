...
class TypeScript implements BaseLang {
    // implemente
    ...
    testLang(debug: boolean): any {
        ...
        if (debug) {
            ...
            const command = `node --require ts-node/register ${actsextension.taskfile} < ${actsextension.tmptestinfile} 1> ${actsextension.tmptestoutfile} 2> ${actsextension.tmptesterrfile}`;
            const options = {
